...
f.theta.RegIndDist = function(K, NbTotalParams, NbParamsModel, theta){
  ...
  for (i in 1:nTheta){
    ...
    for(j in 1:K){
      newTheta[i,ind2:(ind2 + NbParamsModel[j] - 1)] = theta[i,ind1:(ind1 + NbParamsModel[j]-1)] 
      newTheta[i,(ind2 + NbParamsModel[j]):(ind2 + NbParamsModel[j] +NbDistParams[1] - 1)] = DistParams
...
f.theta.RegIndDist.reverse = function(K, NbTotalParams, NbParamsModel, theta){
  ...
  for (i in 1:nTheta){
    ...
    ind2 = 1
    for(j in 1:K){
      newTheta[i,ind1:(ind1 + NbParamsModel[j] - 1)] = theta[i,ind2:(ind2 + NbParamsModel[j]-1)] 
      ind1 = ind1 + NbParamsModel[j]
      ind2 = ind2 + NbParamsModel[j] + length(DistParams)
    }

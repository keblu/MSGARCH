...
#'  The expectation \deqn{\bar{D} = {\mathbf{E}}^{\theta}[D(\theta)]}, where \eqn{{\mathbf{E}}^{\theta}} is the expectation over all theta in the chain,
...
#' \deqn{p_{V} = {\frac{1}{2}}\widehat{var}\left(D(\theta)\right)}, where \eqn{\widehat{var}} is the the standard empirical variance estimator.

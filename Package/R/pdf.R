...
#'pdf = MSGARCH::pdf(object = fit, x = x, log = FALSE, is.its = FALSE)
#'
#'plot(pdf)
#'}
...
#' @export
pdf <- function(object, x, theta, y, log = FALSE, is.its = FALSE)
{
  UseMethod("pdf", object)
}

#' @export
pdf.MSGARCH_SPEC = function(object, x = NULL, theta, y, log = FALSE, is.its = FALSE) {
  ...
  if(isTRUE(is.its)){
    ...
    for(i in 1:nrow(theta)){
      tmp[i,] = object$rcpp.func$pdf_Rcpp(x, theta[i,], y, log)
    }
  }
  out = list()
  out$pdf = tmp
...
#' @export
pdf.MSGARCH_MLE_FIT = function(object, x = NULL, theta = NULL, y = NULL, log = TRUE, is.its = FALSE) {

...
#'                      distributions are  \code{"norm"}, \code{"std"}, and  \code{"ged"}. The vector must be of the same length as the models' vector. \cr (Default: \code{distribution = c("norm", "norm"}))
...
#'                           the same and the distribution parameters do not depend on the regime to which the distribution is attributed.
#'                           If the argument is \code{TRUE}, all distributions in the distribution argument and all skew arguments must be the same. (Default: \code{do.shape.ind = FALSE}) 
...
#' \item \code{is.shape.ind} : Boolean indicating if the distributions' parameters are regime-independent.
...
#' \item \code{ineqlb} : Vector (of size d) of lower inequality bounds.
#' \item \code{inequb} :  Vector (of size d) of upper inequality bounds.
#' \item \code{n.params} :  Vector (of size K) of the total number of parameters by regime including distributions' parameters.
#' \item \code{n.params.vol} :  Vector (of size K) of the total number of parameters by regime excluding distributions' parameters.
...
#'   Under the Haas et al. (2004a) specification, the conditional variance is a function of the past data and the current state.
#'   The Mixture of GARCH option is based on Haas et al. (2004b). A Mixture of GARCH is a mixture of distributions 
...
#'   since it has proved to be sufficient in financial econometrics. This simplification of the processes also  reduces models' complexity which can become a problem during the optimization procedure.

...
#'cdf.its = MSGARCH::cdf(object = fit, log = FALSE, is.its = TRUE)
#' 
#'plot(cdf.its)  
#'                                                                                                                                                                                                                                             
#'# create mesh
...
#'cdf = MSGARCH::cdf(object = fit, x = x, log = FALSE, is.its = FALSE)
#'
#'plot(cdf)
#'}
...
#' @export
cdf <- function(object, x, theta, y, log = FALSE, is.its = FALSE)
{
  UseMethod("cdf", object)
}

#' @export
cdf.MSGARCH_SPEC = function(object, x = NULL, theta, y, log = TRUE, is.its = FALSE) {
  ...
  if(isTRUE(is.its)){
    ...
    for(i in 1:nrow(theta)){
      ...
      if(object$K == 1){
        tmp2 = object$rcpp.func$cdf_Rcpp_its(theta[i,], y, log)
        tmp[i,] = tmp2
      } else{
    ...
    for(i in 1:nrow(theta)){
      tmp[i,] = object$rcpp.func$cdf_Rcpp(x, theta[i,], y, log)
    }
  }
  out = list()
...
#' @export
cdf.MSGARCH_MLE_FIT = function(object, x = NULL, theta = NULL, y = NULL, log = TRUE, is.its = FALSE) {
 
   return(MSGARCH::cdf(object = object$spec, x =  x, theta = object$theta, y = object$y, log = log, is.its = is.its))
  
}

#' @export  
cdf.MSGARCH_BAY_FIT = function(object, x = NULL, theta = NULL, y = NULL, log = TRUE, is.its = FALSE) {

 ...
 */
 ...
 {
   double lambda, g, pdf;
   lambda = sqrt(pow(1.0/2.0,2.0/nu)*Rf_gammafn( 1.0/nu )/Rf_gammafn( 3.0/nu));
 ...
 */
 ...
 double dsgedstd(const double x, const double xi, const double nu)
 {
   ...
   xxi=xi;
   lambda = sqrt(pow(1.0/2.0,(2/nu))*Rf_gammafn( 1.0/nu )/Rf_gammafn( 3.0/nu));
   g = nu/(lambda*(pow(2.0,1.0+(1.0/nu)))*Rf_gammafn( 1.0/nu));
   m1 = pow(2.0, (1.0/nu))*lambda*Rf_gammafn(2.0/nu)/Rf_gammafn(1.0/nu);
   mu = m1*(xi-1.0/xi);
